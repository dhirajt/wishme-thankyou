...
def collect_data():
    ...
    # get user's birthday
    try:
        datafile = urllib.request.urlopen(ACCESS_URL +
...
def reply_post(post_list):
    ...
    reply = input("Enter your reply (enter d for default) : ")
    ...
    post_data = urllib.parse.urlencode({'access_token': ACCESS_TOKEN,
                                  'batch': repr(batch)})

    # POST request since post_data is supplied as second parameter
    dataf = urllib.request.urlopen(ACCESS_URL, post_data)
